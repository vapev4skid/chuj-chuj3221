...
//
//                boolean isOp = player.getScoreboardTeam() != null
//                        && check(player.getScoreboardTeam().getPrefix().getString().toLowerCase());
//
//                boolean isVanished = player.getScoreboardTeam() != null
//                        && player.getScoreboardTeam().getPlayerList().isEmpty();
//
//                if ((isOp || isVanished || gameMode == GameMode.CREATIVE) && !detectedStaff.contains(profile.getName())) {
//
//                    String message = Formatting.RED + String.format("Staff %s Detected!", profile.getName());
//                    Managers.NOTIFICATION.publicity("Staff Detection", message, 2, Notification.Type.WARNING);
//
//                    new Thread(() -> {
