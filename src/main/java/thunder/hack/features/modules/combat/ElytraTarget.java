...
public class ElytraTarget extends Module {
    ...
    public static final boolean WALLS_BYPASS = false;
    public static final int FOV = 360;
    public final Setting<Boolean> fireSpam = new Setting<>("FireSpam", false);
    ...
    public final Setting<Boolean> upDownCrit = new Setting<>("UpDownCrit", false).addToGroup(smartCrit);
    ...
    public final Setting<Float> aimRange = new Setting<>("AimRange", 20f, 0f, 20.0f).addToGroup(advanced);
    ...
    public final Setting<Float> targetDistance = new Setting<>("TargetDistance", 30.0f, 3.0f, 30.0f);
    ...
    private final Timer mimimixTimer = new Timer();
    ...
    private void attackTarget(Entity target) {
        ...
        if (!attackTimer.passedMs(500)) return;

        if (onlySpace.getValue() && !(mc.player.fallDistance > 0.2 && !mc.player.isOnGround())) {
            return;
        }
    ...
    public void resolvePlayers() {
        if (resolver.not(Resolver.Off))
            for (PlayerEntity player : mc.world.getPlayers())
                if (player instanceof OtherClientPlayerEntity && player instanceof IOtherClientPlayerEntityelytra)
                    ((IOtherClientPlayerEntityelytra) player).resolveelytra(resolver.getValue());
    }
    ...
    public void restorePlayers() {
        if (resolver.not(Resolver.Off))
            for (PlayerEntity player : mc.world.getPlayers())
                if (player instanceof OtherClientPlayerEntity && player instanceof IOtherClientPlayerEntityelytra)
                    ((IOtherClientPlayerEntityelytra) player).releaseResolverelytra();
    }
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        ...
        if (upDownCrit.getValue() && mc.player.isFallFlying() && target != null) {
            mc.player.setVelocity(mc.player.getVelocity().x, Math.sin(mc.player.age % 20 < 10 ? -0.2 : 0.2), mc.player.getVelocity().z);
    ...
    public void onSync(EventSync e) {
        ...
        if (target != null && rotationMode.getValue() != Mode.None && rotationMode.getValue() != Mode.Grim) {
            ...
            float[] angles = Managers.PLAYER.calcAngle(targetPos);
    ...
    private void calcRotations(boolean ready) {
        ...
        pitchAcceleration = Managers.PLAYER.checkRtxelytratarget(rotationYaw, rotationPitch, getRange() + aimRange.getValue(), getRange() + aimRange.getValue(), rayTrace.getValue())
        ...
        float delta_yaw = wrapDegrees((float) wrapDegrees(Math.toDegrees(Math.atan2(targetVec.z - mc.player.getZ(), (targetVec.x - mc.player.getX()))) - 90) - rotationYaw) + (!WALLS_BYPASS && !ready && !mc.player.canSee(target) ? 20 : 0);
        ...
        lookingAtHitbox = Managers.PLAYER.checkRtxelytratarget(rotationYaw, rotationPitch, getRange(), getWallRange(), rayTrace.getValue());
    }
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case Exploitcorev2mini -> Exploitcore3.render(target);
            case ThunderHackV2 ->
    ...
    public void onDisable() {
        ...
        if (fireSpam.getValue() && ModuleManager.fireSpam.isEnabled()) {
    ...
    public Vec3d getLegitLook(Entity target) {
        ...
        if (!Managers.PLAYER.checkRtxelytratarget(rotationYaw, rotationPitch, getRange(), getWallRange(), rayTrace.getValue())) {
            ...
            if (PlayerUtility.squaredDistanceFromEyes(target.getPos().add(0, target.getEyeHeight(target.getPose()) / 2f, 0)) <= attackRange.getPow2Value()
                    && Managers.PLAYER.checkRtxelytratarget(rotation1[0], rotation1[1], getRange(), 0, rayTrace.getValue())) {
                // наводим на центр
                ...
                for (float x1 = -halfBox; x1 <= halfBox; x1 += 0.05f) {
                    for (float z1 = -halfBox; z1 <= halfBox; z1 += 0.05f) {
                        for (float y1 = 0.05f; y1 <= target.getBoundingBox().getLengthY(); y1 += 0.15f) {
                            ...
                            if (Managers.PLAYER.checkRtxelytratarget(rotation[0], rotation[1], getRange(), 0, rayTrace.getValue())) {
        ...
        for (float x1 = -halfBox; x1 <= halfBox; x1 += 0.15f) {
            for (float z1 = -halfBox; z1 <= halfBox; z1 += 0.15f) {
                for (float y1 = 0.05f; y1 <= target.getBoundingBox().getLengthY(); y1 += 0.25f) {
                    ...
                    if (Managers.PLAYER.checkRtxelytratarget(rotation[0], rotation[1], (float) Math.sqrt(getSquaredRotateDistance()), getWallRange(), rayTrace.getValue())) {
                        return true;
                    }
                }
            }
    ...
    public Entity findTarget() {
        ...
        for (Entity ent : mc.world.getEntities()) {
            ...
            if (mc.player.squaredDistanceTo(ent.getPos()) <= targetDistance.getValue() * targetDistance.getValue()) {
            ...
            case LowestDurability -> first_stage.stream().min(Comparator.comparing(e -> {
                        ...
                        for (ItemStack armor : e.getArmorItems()) {
                            if (armor != null && !armor.getItem().equals(Items.AIR)) {
                                durability += ((armor.getMaxDamage() - armor.getDamage()) / (float) armor.getMaxDamage());
                            }
                        }
                        return durability;
                    }
    ...
    private boolean skipEntity(Entity entity) {
        ...
        if (!InteractionUtility.isVecInFOV(ent.getPos(), FOV)) return true;
    ...
    public enum Resolver {
        Off, Advantage, Predictive, BackTrack, Mimimix
    }
    ...
    public enum ESP {
        Off, ThunderHack, NurikZapen, CelkaPasta, ThunderHackV2, Exploitcorev1, Exploitcorev2, Exploitcorev2mini
    }
