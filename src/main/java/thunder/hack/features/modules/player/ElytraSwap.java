...
public class ElytraSwap extends Module {
    ...
    public void onUpdate() {
        if (mode.getValue() == Mode.Bind && mc.currentScreen == null) {
            if (switchButton.getValue().getKey() != -1 && isKeyPressed(switchButton.getValue().getKey()) && switchTimer.every(200))
                swapChest(false);

            if (fireWorkButton.getValue().getKey() != -1 && isKeyPressed(fireWorkButton.getValue().getKey()) && fireworkTimer.every(200) && mc.player.isFallFlying())
                useFireWork();
        }
    }
